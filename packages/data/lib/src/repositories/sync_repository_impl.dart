  ...
  Future<List<SyncRecord>> getPendingRecords({int limit = 50}) async {
    ...
    for (final r in received) {
      records.add(SyncRecord(
        ...
        payload: r.toJson(),
        updatedAt: r.date,
        operation: r.syncStatus == SyncStatus.pending ? 'INSERT' : 'UPDATE',
      ));
    }
    ...
    for (final p in payments) {
      records.add(SyncRecord(
        ...
        recordId: p.id!,
        payload: p.toJson(),
        updatedAt: p.createdAt ?? DateTime.now(),
        operation: p.syncStatus == SyncStatus.pending ? 'INSERT' : 'UPDATE',
      ));
    }
      ...
      try {
        ...
        for (final raw in rows as List) {
          try {
            ...
            final error = 'Pull row error in $table (id: ${row['id']}): $e';

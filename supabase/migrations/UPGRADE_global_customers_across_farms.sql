-- ============================================================================
-- ترقية: السماح باستخدام الزبون العام (is_global) عبر المداجن
--
-- ملاحظة: ملف إضافي (additive) - لا يمسح أي بيانات.
-- يُطبَّق في محرر SQL الخاص بـ Supabase (SQL Editor).
--
-- القاعدة: الزبون العادي (is_global=false) يبقى مقيداً بمزرعته كما كان.
--          الزبون العام (is_global=true) يُستخدم في أي مزرعة (تخريج/قبض/مزامنة).
--
-- 1) validate_dispatch_refs : السماح بزبون عام من مزرعة أخرى في طلبات التخريج
-- 2) validate_payment_refs  : السماح بزبون عام من مزرعة أخرى في القبض
-- 3) sync_records_batch     : السماح بزبون عام من مزرعة أخرى أثناء المزامنة
--    (egg_dispatch / dispatch_requests / payments)
-- ============================================================================
...
-- ------------------------------------------------------------
-- (3) المزامنة — يُقبل زبون عام من مزرعة أخرى أثناء رفع السجلات
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- (4) بوابة نطاق الزبون — تحترم القيمة المُرسلة من العميل
--
-- المشكلة السابقة: كان الوسيط يفرض is_global=true تلقائياً على كل زبون
-- يضيفه مدير، فيظهر في كل المداجن رغم أنه مرتبط بمزرعة واحدة.
-- الإصلاح: الزبون يبقى مرتبطاً بمزرعته افتراضياً، ولا يصبح «عاماً»
-- إلا إذا أرسل العميل is_global=true وكان المستخدم مديراً/مدير نظام.
-- ولا يستطيع العامل تغيير نطاق الزبون إطلاقاً.
-- ------------------------------------------------------------

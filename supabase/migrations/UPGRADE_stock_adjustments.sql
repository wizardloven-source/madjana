-- ============================================================
-- UPGRADE: stock_adjustments (تسوية رصيد المخزون: بيض/كرتون/علف)
-- 
-- إضافة جدول تسويات يُزامن كباقي جداول الإدارة (مدير فقط)،
-- مع ربط كل نقاط سلسلة المزامنة: RLS + sync_can_write/read +
-- sync_live_ids + sync_live_exists + sync_records_batch +
-- populate_sync_changes + tombstones.
-- شامل/معاد التنفيذ بأمان على قاعدة موجودة.
-- ============================================================

-- CREATE TABLE IF NOT EXISTS stock_adjustments (deltas for eggs/cartons/feed)
...
-- Farm-scoped manager policy (same hardening as P0 financial tables)
DROP POLICY IF EXISTS mgr_all ON stock_adjustments;
...
-- populate_sync_changes trigger + tombstone trigger for direct deletions
DROP TRIGGER IF EXISTS trg_populate_sync ON stock_adjustments;
CREATE TRIGGER trg_populate_sync AFTER INSERT OR UPDATE OR DELETE ON stock_adjustments
...
-- ============================================================
-- Re-assert sync_can_write (manager list + stock_adjustments)
-- ============================================================
...
-- ============================================================
-- Re-assert sync_can_read (manager list + stock_adjustments)
-- ============================================================
...
-- ============================================================
-- Re-assert sync_live_exists (allowlist + stock_adjustments)
-- ============================================================
...
-- ============================================================
-- Re-assert sync_live_ids (is_global customers + stock_adjustments)
-- ============================================================
...
-- ============================================================
-- Re-assert sync_records_batch (allowed cols + stock_adjustments)
-- ============================================================

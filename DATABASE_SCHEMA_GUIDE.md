...
هذا الدليل يشرح هيكل قاعدة البيانات الموحد للنظام (Supabase). التهيئة تتم من خلال **ملف واحد موحّد** (`supabase/init.sql`) يمثل الحالة النهائية للقاعدة، ويجمع `UNIFIED_schema.sql` مع جميع ملفات `UPGRADE_*.sql` بأحدث التعريفات.

---

## 📁 ملف التهيئة الموحّد

**الموقع:** `supabase/init.sql` (المجلد `supabase/`)  
**الحجم:** ~300KB  
**الحالة:** idempotent — آمن للتشغيل على قاعدة جديدة أو قائمة

### متى تشغّل ماذا؟

| الحالة | الملف الذي يوضَع في SQL Editor |
|--------|-------------------------------|
| **قاعدة جديدة من الصفر** — أول تشغيل لتطبيق جديد ليس فيه بيانات | `supabase/init.sql` يُنشئ كل شيء: الجداول، الدوال، المشغلات، سياسات RLS، الفهارس، ويصبح جاهزاً فوراً |
| **قاعدة قائمة فيها بيانات** — تحديث إلى أحدث نسخة | `supabase/init.sql` نفسه: تحديث فقط (`CREATE TABLE/INDEX ... IF NOT EXISTS`، `ADD COLUMN IF NOT EXISTS`، `CREATE OR REPLACE FUNCTION`، إعادة السياسات/المشغلات بأحدث تعريف) — **لا DROP TABLE، لا حذف بيانات** |

> قاعدة أساسية: **تشغيل `init.sql` على قاعدة لاحقاً غير ضارّ بالبيانات** — إعادة تشغيله أكثر من مرة تنتج الحالة ذاتها (`run twice = same state`).

### مصادر الملف

يُبنى `init.sql` من محتويات `supabase/migrations/` بالترتيب:
`UNIFIED_schema.sql` ثم ملفات `UPGRADE_*.sql` أبجديّاً (الأخير يفوز للدوال والسياسات) — أي تغيير مستقبلي يُضاف كملف `UPGRADE_*.sql` جديد ثم يُعاد توليد `init.sql`.
...
2. افتح `supabase/init.sql` وانسخ محتواه كاملاً
3. الصقه في المحرر واضغط **Run**
4. تحقق من نجاح العملية (لا تظهر أخطاء في الناتج)

> التشغيل نفسه يناسب الحالتين: قاعدة جديدة (يُبني كل شيء) أو قاعدة قائمة (يُحدّث بلا مساس بالبيانات).
...
-- يجب أن تكون النتيجة: 28 جدولاً أو أكثر
```
...
2. افحص ملف `supabase/init.sql` للتفاصيل الكاملة (أو مجلد `supabase/migrations/` للمصادر التطويرية)
3. تحقق من سجل التدقيق (`audit_log`) لفهم التغييرات الأخيرة

---

**آخر تحديث:** 2026-09-19  
**الإصدار:** 4.0 (تهيئة موحّدة عبر `supabase/init.sql`)  
